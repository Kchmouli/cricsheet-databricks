...
- **2026-08-27** — Built `gold.bowling_innings` view (bowler × bowling-innings grain; conventions: wickets=`is_bowler_wicket`, balls=`is_legal_ball`, runs_conceded = bat+wides+noballs+penalty EXCLUDING byes/leg-byes, super-overs excluded). Stores raw additive components (legal_balls/runs_conceded/wickets), never derived rates — rates computed at query time so rollups are correct (can't average an average). Validated vs Cricinfo on Markram: IPL 223 balls/357 runs/6 wkts/9.61 econ/59.50 avg exact; SA20 366/436/18/7.15/24.22 exact. **Key architectural fact: `Mat` (squad appearances) belongs to neither innings view — it comes from `match_player`** (innings views only hold innings where the player batted/bowled). Mat ≥ Inns always. Argues for `player_match_summary` as the authoritative Mat source. Both innings-views now validated; batting + bowling stat coverage complete.
- **2026-09-03** — Built `gold.player_match_summary` (one row per player per squad-match; authoritative Mat; `team_won` for impact/outcome analysis; voided matches excluded). All three semantic views carry full table+column COMMENTs for Genie. **Stood up the Genie agent** (Free Edition) scoped to the three views + dims, with routing/formula/honesty instructions and validated example queries. Genie reproduces Cricinfo numbers exactly and does comparative reasoning (Kohli SR trend, Bumrah 2026 worst-season, Rahul SR-vs-win% impact). Noted limit: Genie's aggregation is trustworthy, its interpretation overclaims causation (honesty-guard instruction mitigates, doesn't eliminate). **Operational hardening:** fixed Bronze daily feed (recently_added_2 + rmtree clear); caught & backfilled a ~1-week CPL 2026 data gap (2-day feed fragility); recommended weekly all_json reconciliation + staleness assert + failure email. **Committed all notebooks + design doc to git and verified a clean full job run (bronze→silver→gold) succeeds from committed code** — resolving the workspace-vs-git drift that caused recurring bugs. Added §17 documenting the semantic layer, Genie config, and hardening. Project is a complete, validated, committed data product with an NL agent.
...
## 17. Semantic layer & Genie agent (built)

The gold semantic layer is complete for batting, bowling, and match-outcome analysis. Three conventions-baked views sit on top of `fact_ball` and are the ONLY things the Genie agent queries for stats (never raw `fact_ball`):

**`gold.batting_innings`** — one row per batter per innings. All six batting conventions applied (super-over excluded, is_ball_faced, non_boundary boundaries, innings = batted-or-dismissed, retired-hurt-not-dismissal; voided matches inherited from fact_ball). Stores runs/balls_faced/fours/sixes/was_out/not_out. Validated exact vs Cricinfo across Kohli/Rahul/Rohit/Markram full IPL careers.

**`gold.bowling_innings`** — one row per bowler per bowling innings. Bowling conventions: wickets=`is_bowler_wicket`, balls=`is_legal_ball`, runs_conceded = bat+wides+noballs+penalty EXCLUDING byes/leg-byes. Stores raw additive components only (never derived rates — rates computed at query time so rollups are correct). Validated exact vs Cricinfo (Markram multi-league; Bumrah full IPL incl. 2026).

**`gold.player_match_summary`** — one row per player per match in the squad. **Authoritative source for Matches Played (Mat)** — squad appearances whether or not the player batted/bowled (batting/bowling-innings undercount Mat and must never be used for match counts). Carries `team_won` (from `won_by_team` = player's team) for win%/impact/outcome analysis. Voided matches excluded.

All three carry full table + column COMMENTs (Genie reads these as context).

### Key architectural facts learned
- **Mat lives only in `player_match_summary`.** Counting matches from the innings views gives "innings batted/bowled in", which equals Mat only for players who bat/bowl in every game (coincidentally right for frontline players, wrong for part-timers — Markram bowled in 24 IPL matches but played 69).
- **Views are create-once DDL, not daily data processing.** They reflect new data automatically. The daily job is bronze→silver→gold only; the Gold Dim Views notebook (dim views + the 3 semantic views) is run by hand when a definition changes. A stray `fact_ball` rebuild cell mistakenly left in that notebook was silently overwriting the fact table on every view run — the root cause of the recurring "vanishing is_ball_faced" bug.

### Genie configuration (live, on Free Edition)
- **Scope:** `batting_innings`, `bowling_innings`, `player_match_summary`, dim views. NOT raw `fact_ball`.
- **Instructions:** vocabulary (names = initials+surname, ILIKE surname match); table-routing (batting→batting_innings, bowling→bowling_innings, Mat/wins→player_match_summary); rate formulas (sum components then divide, never average rates); and an **honesty guard** — win-rate-by-performance splits are associations NOT causation (don't say a player's SR "drives" wins; note conditions cause both; flag flat middle bands; require balls_faced>=10 for SR bands).
- **Example queries:** one validated query per pattern (batting card, bowling card, leaderboards, Mat-by-season, win% by SR band, win% by fifty/wickets).

### Genie validation & limits observed
- Reproduces validated numbers exactly (Kohli SR-by-year, Bumrah 2026 = worst season, all matching Cricinfo).
- **Strength:** retrieval + aggregation are trustworthy (conventions live in the views, so the LLM can't violate them).
- **Weakness:** interpretation. Genie narrates patterns causally by default; the honesty-guard instruction reduces but doesn't eliminate overclaiming (it softens verbs but keeps the story). Treat Genie's *numbers* as reliable and its *editorialising* as a confident draft needing a skeptical read. Text-to-SQL is not a statistician — real causal "impact" analysis (adjusting for venue, innings, opposition) is out of scope.

### Operational hardening
- **Bronze feed fix:** daily feed = `recently_added_2_json.zip` (all-gender); `shutil.rmtree(JSON_DIR)` before each extract. Prior bug: stale files accumulated in the landing dir, so every run reprocessed the whole archive (40-min MERGE); each daily run now processes only its ~80-file delta.
- **Staleness / gap risk:** the 2-day feed is fragile — a missed window loses matches permanently until backfill. Caught a ~1-week CPL 2026 gap (job ran green daily but ingested nothing new after Aug 26); fixed by a manual `all_json` catch-up. Recommended: a weekly `all_json` reconciliation run as a self-healing safety net, plus a staleness assertion (last Gold cell) that fails the job if no data ingested in N days, plus job **email notification on Failure** so stale/broken runs alert instead of failing silently.
- **Git = source of truth:** all four notebooks + design doc committed to github.com/Kchmouli/cricsheet-databricks (`main`). Verified: a clean full job run (bronze→silver→gold) succeeds end-to-end from the committed code. Repeated past bugs traced to workspace-vs-git drift; the fix is always "commit, then run the job from git and confirm green."

### Remaining backlog (optional, additive)
